...
function create(req, res, db) {
    json(req, res, function(req, res) {
        ...
        var cryptedPassword = encryption.digest(user.password + salt); // digest also appends global secret
        db.run('INSERT INTO users (eid, email, firstName, lastName, cryptedPassword, salt VALUES (?,?,?,?,?,?))',
            [user.eid, user.email, user.firstName, user.lastName, crytedPassword, salt], function(err) {
                if (err) {return;}
                res.statusCode(200);
